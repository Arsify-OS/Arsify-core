...
def call_hermes(system: str, prompt: str, timeout: int = 120) -> Optional[str]:
    """Call Hermes Cognitive Engine :8100."""
    try:
        import httpx
        for endpoint in ["/v1/chat/completions", "/chat", "/v1/portsocket"]:
            try:
                r = httpx.post(f"{HERMES_API}{endpoint}", json={
                    "messages": [
                ...
                if r.status_code == 200:
                    d = r.json()
                    result = (d.get("choices", [{}])[0].get("message", {}).get("content") or
                             d.get("response") or d.get("content") or "")
...
def run_senator(domain: str, dry_run: bool = False, test_mode: bool = False) -> dict:
    ...
    if test_mode:
        ...
        print(f"  LLM: Trying Hermes API :8100...")
        response = call_hermes(config["system"], full_prompt)
        ...
        if not response:
            print(f"  LLM: Trying Ollama (slow fallback)...")
